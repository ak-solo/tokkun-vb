...
![1次元配列：インデックスで各要素にアクセスする](images/ch05-array-1d.svg)
...
![2次元配列：行と列をもつ表形式のデータ構造](images/ch05-array-2d.svg)
...
**補足:** 別の関数を呼び出す方法は 6 章で詳しく学びます。ここでは以下のように `Problem5_6()` を呼び出して九九配列を取得し、`x` と `y` に対応する要素を返してください。
